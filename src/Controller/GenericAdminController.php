...
/**
 * Generic Admin Controller configured via frd_admin.entities.
 *
 * This controller provides a zero-code admin interface for entities.
 * Configure entities in config/packages/frd_admin.yaml:
 *
 * frd_admin:
 *     entities:
 *         - Region
...
class GenericAdminController extends AbstractAdminController
{
    public function __construct(
        private readonly array $supportedEntities = [],
    ...
    /**
     * List of supported entities from configuration.
     */
    ...
    {
        // Convert PascalCase entities to readable labels and slugs for the view
        $entities = array_map(function($entityName) {
            return [
                'name' => $entityName,
                // PascalCase to distinct words (e.g. SafetyIncident -> Safety Incident)
                'label' => trim(preg_replace('/[A-Z]/', ' $0', $entityName)),
